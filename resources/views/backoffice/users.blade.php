                        ...
                        @foreach ($users as $user)
                          @if ($user->check == 0)
                            <tr>
                              ...
                              <td>{{$user->description}}</td>
                              <td>
                                {{-- <form action="/valider/{{$user->id}}" method="POST">
                                  @csrf
                                  @method('PUT')
                                  <button class="btn" style="background-color: #AFE3D0">Valider cet utilisateur</button>
                                </form> --}}
                                <a href="/valider/{{$user->id}}" class="btn" style="background-color: #AFE3D0">Valider cet utilisateur</a>
                              </td>
                              <td>
                                <form action="/users/{{$user->id}}" method="POST">
                                  @csrf
                                  @method('DELETE')
                                  <button class="btn" style="background-color: #F7959D">Supprimer cet utilisateur</button>
                                </form>
                              </td>
                            </tr>
                          @endif
